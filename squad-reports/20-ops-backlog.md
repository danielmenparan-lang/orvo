...
**Founder still:** Apply SQL 001→018 on Supabase; set `is_admin`; add Stripe secrets + deploy functions (`STRIPE-DEPLOY-CHECKLIST.md`); flip `ORVO_CHECKOUT_LIVE` when Checkout is live.

**Next code polish:** ~~Connect return URL~~ · ~~builder approve notify~~ · ~~login role home~~ · ~~admin req filter~~ — remaining: live Stripe implementation.
